...
import { Keyword, Competitor, Mention, SourceType, SentimentType, TopicType } from './src/types';
...
const pool = mysql.createPool({
  ...
  database: process.env.MYSQL_DATABASE || 'social_listener',
...
export async function fetchKeywordsFromDB(): Promise<Keyword[]> {
  try {
    const [rows] = await pool.query('SELECT id, keyword as text, category, created_at as createdAt FROM tracked_keywords');
    ...
    console.error('MySQL keywords fetch error:', e);
    return [];
  }
}
...
export async function fetchCompetitorsFromDB(): Promise<Competitor[]> {
  try {
    const [rows] = await pool.query('SELECT id, name, short_name as shortName, website_url as website, created_at as addedAt FROM tracked_competitors');
    return (rows as any[]).map(r => ({
      id: String(r.id),
      name: r.name,
    ...
    console.error('MySQL competitors fetch error:', e);
    return [];
  }
}
...
export async function fetchMentionsFromDB(): Promise<Mention[]> {
  try {
    ...
    console.error('MySQL mentions fetch error:', e);
    return [];
  }
}
...
export async function insertKeywordToDB(keyword: Keyword) {
  try {
    await pool.query('INSERT IGNORE INTO tracked_keywords (keyword, category) VALUES (?, ?)', [keyword.text, keyword.category]);
  } catch (e) {
    console.error(e);
  }
}
...
export async function insertCompetitorToDB(competitor: Competitor) {
  try {
    await pool.query('INSERT IGNORE INTO tracked_competitors (name, short_name, website_url) VALUES (?, ?, ?)', [competitor.name, competitor.shortName, competitor.website]);
  } catch (e) {
    console.error(e);
  }
}
...
export async function deleteKeywordFromDB(id: string) {
  try {
     if (!id.startsWith('kw-')) {
       await pool.query('DELETE FROM tracked_keywords WHERE id = ?', [parseInt(id)]);
     }
  } catch (e) {
    console.error(e);
  }
...
export async function deleteCompetitorFromDB(id: string) {
  try {
     if (!id.startsWith('comp-')) {
       await pool.query('DELETE FROM tracked_competitors WHERE id = ?', [parseInt(id)]);
     }
  } catch (e) {
    console.error(e);
  }
